...
class Human:
    ...
    def on_event(self, event, game: Game):
        if event.type == CIRCLE_ON_BOARD:
            if (
                game.board.grid[event.x][event.y][-1] is not None
                and game.board.grid[event.x][event.y][-1].color.value == game.turn.value
            ):
        ...
        if event.type == pygame.MOUSEBUTTONUP:
                ...
                for j in range(4):
                    if not self.selected:
                        if game.board.grid[i][j][-1] is not None:
                            if (x - (200 + STACK_DISTANCE * i)) ** 2 + (
                                y - (120 + STACK_DISTANCE * j)
                            ) ** 2 <= (9 * game.board.grid[i][j][-1].size.value) ** 2:
                                if (
                                    game.turn.value
                                    == game.board.grid[i][j][-1].color.value
                                ):
                                    self.selected = True
                    else:
                        if Rect(
                            self.left + STACK_DISTANCE * i,
                            self.top + STACK_DISTANCE * j,
                            STACK_DISTANCE,
                            STACK_DISTANCE,
                        ).collidepoint(x, y):
                            if self._j == BLACK_STACK_Y:
                                game.addGobblet(
                                    i,
                                    j,
                                    game.player2.pieces[-(self._i + 2)][-1],
                                    -(self._i + 2),
                                )
                                ...
                                game.addGobblet(
                                    i,
                                    j,
                                    game.player1.pieces[self._i - 4][-1],
                                    self._i - 4,
                                )
            ...
            for i in range(3):
                if game.turn.value == "B":
                    if (x - 100) ** 2 + (y - (280 + (STACK_DISTANCE * i))) ** 2 <= (
                        9 * 4
                    ) ** 2:
                        self.selected = True
    ...
    def on_loop(self, game: Game):
        ...
        for i in range(4):
            for j in range(4):
                if game.board.grid[i][j][-1] is not None:
                    if (x - (200 + STACK_DISTANCE * i)) ** 2 + (
                        y - (120 + STACK_DISTANCE * j)
                    ) ** 2 <= (10 * game.board.grid[i][j][-1].size.value) ** 2:
                        hover = CIRCLE_ON_BOARD
                        MouseHoverevent = pygame.event.Event(
                            hover, x=i, y=j, player=game.turn.value
                        )
        ...
        for i in range(3):
            if (x - 100) ** 2 + (y - (280 + (STACK_DISTANCE * i))) ** 2 <= (9 * 4) ** 2:
                hover = BLACK_STACKS
                MouseHoverevent = pygame.event.Event(
                    hover, x=-2 - i, player=game.turn.value
                )
                pygame.event.post(MouseHoverevent)
        for i in range(3):
            if (x - 540) ** 2 + (y - (40 + (STACK_DISTANCE * (2 - i)))) ** 2 <= (
                9 * 4
            ) ** 2:
                hover = RED_STACKS
                MouseHoverevent = pygame.event.Event(
                    hover, x=4 + i, player=game.turn.value
                )
                pygame.event.post(MouseHoverevent)
