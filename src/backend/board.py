...
class Board:
    ...
    def addPiece(self, row: int, column: int, piece: Piece, player: Player, external_row: int):
        ...
        if self.grid[row][column][-1] is None:
            ...
            if len(linedUpGobblets) > 0 and len(
                    [gobblet for gobblet in linedUpGobblets if (row, column) in gobblet]) > 0:
    ...
    def getLinedUpGobblets(self, color: Color) -> list[list[tuple[int, int]]]:
        ...
        # Scan diagonals
        def scanDiagonal(startRow: int, startColumn: int, rowLimit: int, columnLimit: int, acceptedLength: int,
                         leftDiagonal: bool):
            temp = []
            row = startRow
            column = startColumn
